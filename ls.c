...
/*True means that the files should be displayed in a recursive tree*/
static bool tree = false;

static int depth_arg = 1;
...
static struct option const long_options[] =
{
  ...
  {"depth", optional_argument, NULL, 'e'},
  ...
  {"tree-recursive", no_argument, NULL, 'M'},
...
static enum depth_val const depth_args[]=
{
  d0,d1,d2,d3,d4,d5,d6,d7,d8
};

static char const *const depth_strings[] =
{
  "1","2,","3","4","5","6","7","8","9", NULL
};
ARGMATCH_VERIFY(depth_strings, depth_args);
  ...
  while (true)
    {
      ...
      int c = getopt_long (argc, argv,
                           "abcdefghiklmnopqrstuvw:xABCDFGHI:LMNQRST:UXZ1",
                           long_options, &oi);
      if (c == -1)
        break;
      ...
      switch (c)
        ...
        case 'e':
          if(optarg){
            depth_arg = XARGMATCH("--depth", optarg, depth_strings, depth_args);
            //if(depth_arg >=2){depth_arg++;}
          }
          else{
            depth_arg = 1;
          }
                ...
                || ! basename_is_dot_or_dotdot (f->name)))
          {
            if (!dirname || f->name[0] == '/')
            ...
            else
              {
                ...
                //Finds the depth we are currently at by checking the dirname
                // for the number of '/' it contains. Only queues that many dirs
                int x = 0;
...
"), stdout);
  ...
  -M  --depth                print the folder in easily readable format to\n\
                             desired depth\n\
"), stdout);
      fputs (_("\
